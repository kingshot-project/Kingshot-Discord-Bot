...
def make_js_db(path):
    ...
    conn.executescript("""
        CREATE TABLE alliance (
            ...
            game_type TEXT NOT NULL DEFAULT 'ks',
...
def seed_basic_js(js):
    js.execute("INSERT INTO alliance (id, game_type, priority, name, guide_id, channel_id, interval, auto_redeem) "
               "VALUES (1, 'ks', 1, 'Alpha', '111', '222', '60', 1)")
    js.execute("INSERT INTO alliance (id, game_type, priority, name, guide_id, channel_id, interval, auto_redeem) "
               "VALUES (2, 'wos', 1, 'KsOnly', '111', '333', '60', 0)")
    js.execute("INSERT INTO players (game_type, fid, user_id, nickname, furnace_level, state, alliance_id, added_by) "
               "VALUES ('ks', 100, '900', 'PlayerA', 30, 555, 1, 'x')")
    js.execute("INSERT INTO players (game_type, fid, user_id, nickname, furnace_level, state, alliance_id, added_by) "
               "VALUES ('wos', 200, NULL, 'KsPlayer', 10, 7, 2, 'x')")
    js.execute("INSERT INTO admins (user_id, alliances, is_owner) VALUES ('42', '[1]', 1)")
    js.execute("INSERT INTO gift_codes (game_type, gift_code, date, status) VALUES ('ks', 'CODE1', '2025-01-01', 'valid')")
    js.execute("INSERT INTO giftcode_usage (game_type, fid, gift_code, status) VALUES ('ks', 100, 'CODE1', 'success')")
    js.execute("INSERT INTO id_channels (game_type, guide_id, alliance_id, channel_id, linked_by) "
               "VALUES ('ks', '111', 1, '444', '42')")
    js.execute("INSERT INTO furnace_changes (game_type, fid, old_furnace_lv, new_furnace_lv, change_date) "
               "VALUES ('ks', 100, 29, 30, '2025-01-02T03:04:05.000Z')")
    js.execute("INSERT INTO nickname_changes (game_type, fid, old_nickname, new_nickname, change_date) "
               "VALUES ('ks', 100, 'Old', 'PlayerA', '2025-01-02T03:04:05.000Z')")
    js.execute("INSERT INTO test_ids (game_type, id, fid, state) VALUES ('ks', 1, 777, 555)")
    js.commit()
...
def test_fresh_import_copies_wos_rows_and_skips_other_game(tmp_path):
    ...
    stats = bji.run_import(str(js_path), str(tmp_path / "db"), game_type="ks")
...
def test_colliding_alliance_id_gets_remapped(tmp_path):
    ...
    js.execute("INSERT INTO alliance (id, game_type, priority, name) VALUES (1, 'ks', 1, 'Bravo')")
    js.execute("INSERT INTO players (game_type, fid, nickname, alliance_id, added_by) "
               "VALUES ('ks', 300, 'BravoPlayer', 1, 'x')")
    js.commit()
    make_target(tmp_path / "db")
...
def test_alliance_matched_by_name_keeps_local_settings(tmp_path):
    ...
    js.execute("INSERT INTO alliance (id, game_type, priority, name, channel_id, interval) "
               "VALUES (5, 'ks', 1, 'Alpha', '999', '30')")
    js.execute("INSERT INTO players (game_type, fid, nickname, alliance_id, added_by) "
               "VALUES ('ks', 400, 'AlphaGuy', 5, 'x')")
    js.commit()
    make_target(tmp_path / "db")
    with closing(sqlite3.connect(tmp_path / "db" / "alliance.sqlite")) as conn:
...
def test_rerun_is_idempotent(tmp_path):
    ...
    bji.run_import(str(js_path), str(tmp_path / "db"), game_type="ks")
    stats2 = bji.run_import(str(js_path), str(tmp_path / "db"), game_type="ks")
...
def test_dry_run_reports_counts_without_writing(tmp_path):
    ...
    stats = bji.run_import(str(js_path), str(tmp_path / "db"), game_type="ks", dry_run=True)
...
def test_null_nickname_and_furnace_get_defaults(tmp_path):
    js_path = tmp_path / "Database.db"
    js = make_js_db(js_path)
    js.execute("INSERT INTO alliance (id, game_type, priority, name) VALUES (1, 'ks', 1, 'Alpha')")
    js.execute("INSERT INTO players (game_type, fid, nickname, furnace_level, alliance_id, added_by) "
               "VALUES ('ks', 600, NULL, NULL, 1, 'x')")
    js.commit()
    make_target(tmp_path / "db")
...
def test_duplicate_js_alliance_names_merge_locally(tmp_path):
    ...
    js = make_js_db(js_path)
    js.execute("INSERT INTO alliance (id, game_type, priority, name) VALUES (1, 'ks', 1, 'Alpha')")
    js.execute("INSERT INTO alliance (id, game_type, priority, name) VALUES (2, 'ks', 2, 'Alpha')")
    js.execute("INSERT INTO players (game_type, fid, nickname, alliance_id, added_by) VALUES ('ks', 700, 'A', 1, 'x')")
    js.execute("INSERT INTO players (game_type, fid, nickname, alliance_id, added_by) VALUES ('ks', 701, 'B', 2, 'x')")
    js.commit()
    make_target(tmp_path / "db")
